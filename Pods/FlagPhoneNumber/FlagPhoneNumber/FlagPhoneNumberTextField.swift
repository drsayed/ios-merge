...
open class FPNTextField: UITextField, FPNCountryPickerDelegate, FPNDelegate {
	...
	public var flagSize: CGSize = CGSize(width: 30, height: 30) {
		didSet {
			layoutSubviews()
		}
	}
	...
	private func setupPhoneCodeTextField() {
        phoneCodeTextField.font = UIFont(name: "HelveticaNeue", size: 15)               //font size: 17
        phoneCodeTextField.resignFirstResponder()
//        let bottomLine = CALayer()
//        bottomLine.frame = CGRect(x: 0, y: phoneCodeTextField.frame.height - 1, width: phoneCodeTextField.frame.width, height: 1)
//        bottomLine.backgroundColor = UIColor.white.cgColor
//        phoneCodeTextField.borderStyle = .none
//        phoneCodeTextField.layer.addSublayer(bottomLine)
	...
	private func updateUI() {
		...
		if let phoneCode = selectedCountry?.phoneCode {
            let downArrow = "▼"
			phoneCodeTextField.text = phoneCode + "  " + downArrow
...
//        }
	...
	func countryPhoneCodePicker(_ picker: FPNCountryPicker, didSelectCountry country: FPNCountry) {
        (delegate as? FPNTextFieldDelegate)?.fpnDidSelectCountry(name: country.name, dialCode: country.phoneCode, code: country.code.rawValue, textField: self)
