...
- (void)sendMessageWithBody:(NSString *)messageBody uColor:(NSString *)color uName:(NSString *)name userId:(NSString *)from uimage:(NSString *)image urank:(NSString *)rank
{
    ...
    NSXMLElement *data = [NSXMLElement elementWithName:@"data" xmlns:@"my-custom-data-ns"];
    if (color)
        [data addAttributeWithName:@"uColor" stringValue:color];
    if (name)
        [data addAttributeWithName:@"uName" stringValue:name];
    if (from)
        [data addAttributeWithName:@"userId" stringValue:from];
    if (image)
        [data addAttributeWithName:@"uImage" stringValue:image];
    if (rank)
        [data addAttributeWithName:@"uRank" stringValue:rank];
