...
import AVFoundation
import Photos
protocol LandScapVideoDelegate:class {
...
//           videoView.addGestureRecognizer(videoTap)
    ...
    override func layoutSubviews() {
           self.videosCollection.layoutIfNeeded()
          // [self.videosCollections, layoutIfNeeded]
       }
    ...
    func removePeriodicTimeObserver() {
       ...
       func getThumbnailImageFromVideoUrl(url: URL, completion: @escaping ((_ image: UIImage?)->Void)) {
           DispatchQueue.global().async { //1
               ...
               avAssetImageGenerator.maximumSize = CGSize(width: self.width, height: 500)
...
extension LandScapVideoCell : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout
{
...
//        }
    ...
    //                let videoCell = cell as! LandScapCell
    //                videoCell.playerView.isMuted = true
    //                if  videoCell.playerView.isMuted {
    //                    videoCell.muteBtn.setImage(tintMuteImg, for: .normal)
    //                } else {
    ...
    //        }
        ...
        delegateVideoChange?.LandScapVideoChanged()
...
//                let tintUnmuteImg = unMuteImg.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
//
//                if let videoCell = self.videosCollection.cellForItem(at: indexPathPre) as? LandScapCell {
...
//                }
//
//                if let videoCell1 = self.videosCollection.cellForItem(at: indexPathNext) as? LandScapCell {
...
//            if let videoCell = self.videosCollection.cellForItem(at: visibleCellIndex) as? LandScapCell {
//
//                        videoCell.playerView.isMuted = true
//                        if  videoCell.playerView.isMuted {
//                            videoCell.muteBtn.setImage(tintMuteImg, for: .normal)
...
//
//            if let portrateCell = self.collectionView.cellForItem(at: centerIndexPath) as? EmplLandScapCell {
...
extension LandScapVideoCell : LandScapFullScreenDelegate
{
    func LandScapVideoFullScreenPressed(player: AVPlayer) {
        delegateVideoChange?.LandScapVideoFullScreenPressed(player: player)
    }
