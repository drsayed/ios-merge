...
class FeedsCMDetailsVC: BaseCVC , FriendControllerDelegate {
    ...
    var commentDataSource = [CommentCMItem]()
    ...
    private func setupCollectionView(){
        ...
        collectionView?.register(DetailCMCell.Nib, forCellWithReuseIdentifier: DetailCMCell.identifier)
    ...
    fileprivate func getDetails(){
        service.getCMDetails(cmId: cmId)
    }
    ...
    func insertComment(comment: String, editId: String? = nil){
        ...
        if let window = UIApplication.shared.keyWindow {
            ...
            service.insertEditCMComment(cmId: cmId, commentText: comment, editCommentId: editId) {[weak self] (callbackComment) in
    ...
    static func controllerInstance(cmId: String) -> FeedsCMDetailsVC {
        let layout = UICollectionViewFlowLayout()
        let vc = FeedsCMDetailsVC(collectionViewLayout: layout)
        vc.cmId = cmId
        return vc
    }
}
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.section == 0 {
            ...
            case .feedVideoTextCell:
                return UICollectionViewCell()
            case .companyMonth:
                guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: DetailCMCell.identifier, for: indexPath) as? DetailCMCell else { return UICollectionViewCell()}
                cell.item = data
            ...
            cell.configCMCell(item: commentDataSource[indexPath.item])
            cell.delegate = self
            return cell
        }
    }
...
extension FeedsCMDetailsVC: UICollectionViewDelegateFlowLayout{
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        if indexPath.section == 0 {
            ...
            case .feedVideoTextCell:
                return CGSize(width: 0, height: 0)
            case .companyMonth:
                let height = FeedsHeight.heightForDetailCompanyOfMonthCellV2(item: item, labelWidth: width - 16)  //Aligning width by omitting (leading & trailing)
            ...
            let height = FeedsHeight.heightForCommentCompanyOfMonthCellV2(item: commentDataSource[indexPath.item], labelWidth: self.view.frame.width)
...
extension FeedsCMDetailsVC {
  ...
  extension FeedsCMDetailsVC: DetailDelegate{
    ...
    func DidReceivePOWDetails(feed: FeedV2Item?, comment: [CommentPOWItem]) {
        print("POW will not get fired in CM")
    }
...
extension FeedsCMDetailsVC: MSInputViewDelegate{
...
extension FeedsCMDetailsVC: UpdatedFeedsDelegate{
  ...
  extension FeedsCMDetailsVC: CommentCollectionCellDelegate{}
