...
import Foundation
public class LiveInputView: UIView{
    ...
    private var profileImgLeadingAnchor: NSLayoutConstraint?
    ...
    lazy var profileView : CircleView = {
        ...
        print("Color code : \(AuthService.instance.colorCode)")
        pv.backgroundColor = UIColor(hexString: AuthService.instance.colorCode)
        return pv
    }()
    ...
    lazy var initialLbl : UILabel = {
        ...
        lbl.textColor = .white
        lbl.text = AuthService.instance.initial
    ...
    private func setupViews(){
        ...
        profileView.addSubview(initialLbl)
        ...
        //Circle View
        profileView.setLeading(to: leadingAnchor, constant: 8)
        profileView.frame = CGRect(x: 8, y: 8, width: 40, height: 40)
        //profileView.setAnchors(leading: leadingAnchor, trailing: nil, top: topAnchor, bottom: bottomAnchor, padding: UIEdgeInsets(top: 8, left: 8, bottom: 8, right: 0))
        profileView.setSize(size: CGSize(width: 40, height: 40))
        profileView.setVerticalCenter(to: self)
        //Initial Label
        initialLbl.width = 30
        initialLbl.height = 15
        initialLbl.x = 10
        initialLbl.y = 13
        //Profile image
        profileImageView.frame = CGRect(x: 0, y: 0, width: 40, height: 40)
        profileImageView.sd_setImage(with: URL(string: AuthService.instance.profilePic), completed: nil)
        
        //TextviewBackground
        bgView.setAnchors(leading: profileView.trailingAnchor, trailing: nil, top: topAnchor, bottom: bottomAnchor, padding: UIEdgeInsets(top: 8, left: 8, bottom: 8, right: 0))
        //TextView
        ...
        sendButton.setVerticalCenter(to: profileView)
    }
    ...
    open func calculateIntrinsicContentSize() -> CGSize {
        ...
        let maxTextViewSize = CGSize(width: inputTextView.bounds.width, height: CGFloat(MAXFLOAT))
        var inputTextViewHeight = inputTextView.sizeThatFits(maxTextViewSize).height.rounded()
        print("Assigned : \(inputTextViewHeight) , \(maxTextViewHeight)")
