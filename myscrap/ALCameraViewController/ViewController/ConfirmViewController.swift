...
public class ConfirmViewController: UIViewController, UIScrollViewDelegate {
	...
	private func makeProportionalCropRect() -> CGRect {
        print("OuterGap : \(cropOverlay.outterGap), crop frame height :\(cropOverlay.frame.size.height)")
        let cropHeight = cropOverlay.frame.size.height - 100
		var cropRect = CGRect(x: cropOverlay.frame.origin.x + cropOverlay.outterGap,
		                      ...
		                      height: cropHeight - 2 * cropOverlay.outterGap)
        ...
        print("Crop Rect x : \(cropRect.origin.x), y : \(cropRect.origin.y)")
        print("Image view width : \(imageView.frame.width) , height :\(imageView.frame.height)")
        print("CropRect width : \(cropRect.width), height : \(cropRect.height)")
		let normalizedWidth = cropRect.width / imageView.frame.width
		let normalizedHeight = cropRect.height / imageView.frame.height
        print("Final values : \(normalizedX), \(normalizedY), \(normalizedWidth), \(normalizedHeight)")
		return CGRect(x: 0.19, y: normalizedY, width: 0.6, height: 0.5) //Perfect 
	}
