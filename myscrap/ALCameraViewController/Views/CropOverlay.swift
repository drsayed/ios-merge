...
internal class CropOverlay: UIView {
    ...
    override func layoutSubviews() {
        let bouHeight = bounds.height - 100
        print("OuterGap : \(outterGap)")
        for i in 0..<outerLines.count {
            ...
            case 0:
                ...
                break
            case 1:
                lineFrame = CGRect(x: bounds.width - lineWidth - outterGap, y: outterGap, width: lineWidth, height: bouHeight - outterGap * 2)
                break
            case 2:
                lineFrame = CGRect(x: outterGap, y: bouHeight - lineWidth - outterGap, width: bounds.width - outterGap * 2, height: lineWidth)
                break
            case 3:
                lineFrame = CGRect(x: outterGap, y: outterGap, width: lineWidth, height: bouHeight - outterGap * 2)
                break
            default:
        ...
        for i in 0..<corners.count {
			...
			case 2:	// Bottom Left
				verticalFrame = CGRect(x: outterGap, y:  bouHeight - cornerLineWidth - outterGap, width: cornerLineDepth, height: cornerLineWidth)
				horizontalFrame = CGRect(x: outterGap, y:  bouHeight - cornerLineDepth - outterGap, width: cornerLineWidth, height: cornerLineDepth)
				buttonFrame = CGRect(origin: CGPoint(x: 0, y: bouHeight - cornerButtonWidth), size: buttonSize)
			case 3:	// Bottom Right
				verticalFrame = CGRect(x: bounds.width - cornerLineDepth - outterGap, y: bouHeight - cornerLineWidth - outterGap, width: cornerLineDepth, height: cornerLineWidth)
				horizontalFrame = CGRect(x: bounds.width - cornerLineWidth - outterGap, y: bouHeight - cornerLineDepth - outterGap, width: cornerLineWidth, height: cornerLineDepth)
				buttonFrame = CGRect(origin: CGPoint(x: bounds.width - cornerButtonWidth, y: bouHeight - cornerButtonWidth), size: buttonSize)
            ...
            print("Horizontal frame : \(horizontalFrame)")
            print("Vertical frame : \(verticalFrame)")
        }
		
		let lineThickness = lineWidth / UIScreen.main.scale
		let vPadding = (bouHeight - outterGap * 2 - (lineThickness * CGFloat(horizontalLines.count))) / CGFloat(horizontalLines.count + 1)
        ...
        for i in 0..<horizontalLines.count {
			...
			//vLine.frame = CGRect(x: hSpacing + outterGap, y: outterGap, width: lineThickness, height: bouHeight - outterGap * 2)
        }
        print("Bounds height : \(bouHeight), width : \(bounds.width)")
