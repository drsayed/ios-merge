//
//  UserFeedLandScapVideoTextCell.swift
//  myscrap
//
...
import AVFoundation
import Photos
protocol UserFeedLandScapTextVideoDelegate:class {
    func UserFeedLandScapTextVideoChanged()
    func UserLandScapVideoTextFullscreenPressed(player: AVPlayer)
}
class UserFeedLandScapVideoTextCell: BaseCell {
    weak var delegateVideoChange : UserFeedLandScapTextVideoDelegate?
...
extension UserFeedLandScapVideoTextCell : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout
{
...
//            if currentPage != visibleCellIndex.item {
//
//                let muteImg = #imageLiteral(resourceName: "mute-60x60")
//                let tintMuteImg = muteImg.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
//
...
//            }
      //  NotificationCenter.default.post(name: Notification.Name("VideoPlayedChanged"), object: nil)
        delegateVideoChange?.UserFeedLandScapTextVideoChanged()
...
extension UserFeedLandScapVideoTextCell : LandScapFullScreenDelegate
{
    func LandScapVideoFullScreenPressed(player: AVPlayer) {
        delegateVideoChange?.UserLandScapVideoTextFullscreenPressed(player: player)
    }
