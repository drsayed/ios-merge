                ...
                Auth.auth().signIn(with: credential) { authData, error in
                    if ((error) != nil) {
                      ...
                      //  return
                        ...
                        EditProfileItem.updateProfileAfterVerify(Param: self.editDataString,completion: {[weak self] (success, message) in
                            if success {
                                ...
                                if let loggedinEmail = UserDefaults.standard.object(forKey: "storedEmail") as? String {
                                    if loggedinEmail != self!.email {
                                        UserDefaults.standard.set(self?.email, forKey: "storedEmail")
                                    }
                                }
                                UserDefaults.standard.set("1", forKey: "MobileEdited" + AuthService.instance.userId)
...
extension EditProfileCodeVerifyVC : UITextFieldDelegate {
    func textFieldDidBeginEditing(_ textField: UITextField) {
        textField.text = ""
    }
}
extension EditProfileCodeVerifyVC: AuthServiceDelegate{
