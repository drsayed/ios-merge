//
//  ReportedLandscapVideoCell.swift
//  myscrap
//
...
import AVFoundation
import Photos
protocol VideoOnLandscapReportedPlayerDelegate : class {
...
class ReportedLandscapVideoCell: BaseCell {
 ...
 @IBOutlet weak var videosCollection: UICollectionView!
    weak var videoPlayerDelegate : VideoOnLandscapReportedPlayerDelegate?
 ...
 var addCommentAction : (() -> Void)? = nil
    ...
    func configCell(withItem item: FeedV2Item) {
        ...
        profileTypeView.checkType = ProfileTypeScore(isAdmin:false,isMod: false, isNew:item.newJoined, rank:nil, isLevel: false, level: nil)
...
//    }
    ...
    @objc  func playButtonPresssed(sender: UIButton!){
       //...
        ...
        videoPlayerDelegate?.playLandscapReportedVedio(url: objVide.video)
    }
}
extension ReportedLandscapVideoCell : UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout
{
...
return CGSize(width:screenWidth, height: 420)
    ...
    let tintUnmuteImg = unMuteImg.withRenderingMode(UIImage.RenderingMode.alwaysOriginal)
        ...
        videoPlayerDelegate?.LandscapVideoChanged()
...
extension ReportedLandscapVideoCell : LandScapFullScreenDelegate
{
    func LandScapVideoFullScreenPressed(player: AVPlayer) {
       videoPlayerDelegate?.ReportedLandscapVideoFullscreenPressed(player: player)
    }
