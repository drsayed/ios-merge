...
//        }
    ...
    func setCurrentPosition(position: Int){
        ...
        if iOS14 {
            ...
            DispatchQueue.main.async {
                let pageOffset = CGPoint(
                  // for landscape mode
                    x: Int((self.view.bounds.width)) * self.currentPosition ,
                  y: 0
                )
                
                self.collectionPage.setContentOffset(pageOffset, animated: true)
    ...
    private class HeaderCell: UICollectionViewCell {
        ...
        private func setupUI(){
            // view
            indicator.backgroundColor = UIColor(red: 0.00, green: 0.50, blue: 0.00, alpha: 1.00)//.green
