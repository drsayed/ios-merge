//
//  EditCommentCMVC.swift
//  myscrap
//
//  Created by MyScrap on 7/18/19.
//  Copyright © 2019 MyScrap. All rights reserved.
//

import UIKit

class EditCommentCMVC: UIViewController {
    ...
    typealias closure = (CommentCMItem) -> ()
    
    var commentClosure: closure?
    
    var item : CommentCMItem?
    ...
    override func viewDidLoad() {
        ...
        scrollView.text = item?.commentText
    ...
    @IBAction func sendButtonPressed(_ sender: UIButton){
        item?.commentText = scrollView.text
        if let cls = commentClosure, let it = item, it.commentText != "" {
            cls(it)
            dismiss(animated: true, completion: nil)
        }
    }
    
    static func storyBoardInstance() -> EditCommentCMVC? {
        return UIStoryboard.Comment.instantiateViewController(withIdentifier: EditCommentCMVC.id) as? EditCommentCMVC
    }
