...
import UIKit
import SDWebImage
protocol ImageSwipeAlbumCVDelgate: class {
    func showingImage(image:UIImage)
}
class MyScrapAlbumCell: BaseCVCell{
    var rightBarActionBlock: (() -> Void)? = nil
    weak var delegate: ImageSwipeAlbumCVDelgate?
...
//        buttonMenu.addTarget(self, action: #selector(rightBarPressed), for: .touchUpInside)
        ...
        imageView.anchor(leading: scrollView.leadingAnchor, trailing: scrollView.trailingAnchor, top: scrollView.topAnchor, bottom: scrollView.bottomAnchor)
...
extension MyScrapAlbumCell: UIScrollViewDelegate{
